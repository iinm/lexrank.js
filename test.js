...
var stopwords = {};
_stopwords.forEach(function(w) {
  stopwords[w] = true;
});

var _segmenter = new TinySegmenter();
var word_segmenter = function(sent) {
  var _words = _segmenter.segment(sent);
  var words = [];
  for (var i = 0; i < _words.length; i++) {
    var w = _words[i].trim();
    if (stopwords[w] == null) words.push(w);
  }
  return words;
}
...
//console.log(result);
console.log('# (score)  sentence');
result.forEach(function(sent) {
  console.log('(' + sent.score + ')  ' + sent.sent);
});
